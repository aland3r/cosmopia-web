...
// into vendor/gestalt/ so `milebrick-web` can build standalone (e.g. on Vercel,
...
if (!fs.existsSync(path.join(monorepoRoot, 'ui/auth/index.js'))) {
  console.log('[sync-vendor] monorepo not found next to milebrick/, skipping (using existing vendor/gestalt)')
  process.exit(0)
}
